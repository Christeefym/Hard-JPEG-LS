...
**JPEG-LS** （简称**JLS**）是一种无损/有损的图像压缩算法，其无损模式的压缩率相当优异，优于 Lossless-JPEG、Lossless-JPEG2000、Lossless-JPEG-XR、FELICES 等。**JPEG-LS** 用压缩前后的像素的最大差值（NEAR值）来控制失真，无损模式下 NEAR=0；有损模式下NEAR>0，NEAR 越大，失真越大，压缩率也越大。**JPEG-LS** 压缩图像的文件后缀是 .**jls** 。

# 使用方法

[**RTL/jls_encoder.sv**](https://github.com/WangXuan95/Hard-JPEG-LS/blob/master/RTL/jls_encoder.sv) 是用户可以调用的 JPEG-LS 压缩模块，它输入图像原始像素，输出 JPEG-LS 压缩流。它的接口描述如下表。

| 信号名称 | 全称 | 方向 | 宽度 | 描述 |
| :---: | :---: | :---: | :---: | :--- |
| rstn | 同步复位 | input | 1bit | 当时钟上升沿时若 rstn=0，模块复位，正常使用时 rstn=1 |
| clk | 时钟 | input | 1bit | 时钟，所有信号都应该于 clk 上升沿对齐。 |
| i_sof | 图像开始 | input | 1bit | 当需要输入一个新的图像时，保持至少13个时钟周期的 i_sof=1 |
...
1. **复位**（可选）：令 rstn=0 至少 **1 个周期**进行复位，之后正常工作时都保持 rstn=1。实际上可以不复位（即让 rstn 恒为1）。
2. **开始**：保持 i_sof=1 **至少 13 个周期**，同时在 i_w 和 i_h 信号上输入图像的宽度和高度，i_sof=1 期间 i_w 和 i_h 要一直保持有效。
...
- 将 [**RTL/tb_jls_encoder.sv**](https://github.com/WangXuan95/Hard-JPEG-LS/blob/master/RTL/tb_jls_encoder.sv) 里的宏名 **INPUT_PGM_DIR** 和 **OUTPUT_JLS_DIR** 分别改成你的计算机里的 **images** 和 **images_jls** 文件夹的路径。注意！Windows下的目录分隔符为\，但因为 Verilog 字符串需要转义，所以这里的分隔符是\\。
- 运行仿真，运行时间可以设置的很长，压缩完所有图像后它会遇到 $stop; 而停止。
- **images_jls** 文件夹里出现压缩后的 .jls 文件。
- 你可以修改 [**RTL/tb_jls_encoder.sv**](https://github.com/WangXuan95/Hard-JPEG-LS/blob/master/RTL/tb_jls_encoder.sv) 里的宏名 **NEAR** 来改变压缩率。
- 你可以修改 [**RTL/tb_jls_encoder.sv**](https://github.com/WangXuan95/Hard-JPEG-LS/blob/master/RTL/tb_jls_encoder.sv) 里的宏名 **NUM_BUBBLES** 来决定相邻像素间插入多少个气泡。默认为0，即不插入气泡。
...
因为 **JPEG-LS** 比较小众和专业，大多数图片查看软件无法查看 .jls 文件。可以用我提供的解码器 [**decoder.exe**](https://github.com/WangXuan95/Hard-JPEG-LS/blob/master/decoder.exe) 来把它解码回 .pgm 文件再查看。用 CMD 运行解码命令：
...
> 注：decoder.exe 编译自不列颠哥伦比亚大学提供的 JPEG-LS 解码器源码： http://www.stat.columbia.edu/~jakulin/jpeg-ls/mirror.htm
...
|    LUT     |     FF      |    BRAM    | 最高时钟频率 |
| :--------: | :---------: | :--------: | :----------: |
| 5280 (34%) | 13511 (32%) | 16 KB (8%) |    32 MHz    |

32MHz 下，图像压缩的性能为 32 Mpixel/s ，对 1920x1080 图片的帧率是 15fps 。
